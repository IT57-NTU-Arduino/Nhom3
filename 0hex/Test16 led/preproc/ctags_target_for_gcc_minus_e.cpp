# 1 "D:\\Download\\GIT FOR VERSION CONTROL\\Quan ly File Of Git\\Arduino FILE\\Test16 led\\Test16_led\\Test16_led.ino"
# 1 "D:\\Download\\GIT FOR VERSION CONTROL\\Quan ly File Of Git\\Arduino FILE\\Test16 led\\Test16_led\\Test16_led.ino"
/*
...
void TshiftOutHC595(int dataPin, int clockPin, byte ledStatus[]) {
  ...
  for (int i = 0; i < HC595_COUNT; i++) {
    shiftOut(dataPin,clockPin,1,ledStatus[i]);
  }
...
void PshiftOutHC595(int dataPin, int clockPin, byte ledStatus[]) {
  ...
  for (int i = 0; i < HC595_COUNT; i++) {
    shiftOut(dataPin,clockPin,0,ledStatus[i]);
  }
